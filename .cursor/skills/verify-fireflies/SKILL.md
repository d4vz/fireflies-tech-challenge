...
Drive the Next.js UI the way a user does. Do not call Hono on port 3000 from the browser, and do not open `http://127.0.0.1:8080` (that is the operator's session).
...
- `ui_url=http://127.0.0.1:18080` (or the port you set), owned by the recorded UI pid
...
- `GET <ui_url>/api/meetings?page=1&limit=5` 200 JSON with `items`
...
2. `browser_navigate` to `ui_url` (Home `/`).
...
- a screenshot of the resulting screen with `Davi` or the `Home` / `Meetings` heading visible (`browser_take_screenshot`, `filename` the artifact png)
