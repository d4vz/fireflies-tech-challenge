...
- **Open from list.** From `/meetings`, choose the row named the meeting `name`. Run `browser_click` that link. The article `h1` is that `name`. Status is one of `Queued`, `Processing`, `Ready`, `Failed` (the chip uses uppercase style).
...
- **Unknown id.** Open `/meetings/missing`. Run `browser_navigate` to `{ui_url}/meetings/missing`. Main copy is `meeting not found` inside a destructive alert.
