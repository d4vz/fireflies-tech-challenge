...
Home is `/`. After Clerk sign-in it greets the session first name (`Verify` on a launch-created user), shows insight cards for the current library sample, filters that sample with All / Ready / Busy / Failed tabs and a `sourceId`-or-summary search, and links through to the full meetings list.
