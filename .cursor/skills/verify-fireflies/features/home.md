...
Home is `/`. After Clerk sign-in it greets the session first name (`Verify` on a launch-created user) with a trailing `👋`, shows insight cards for the current library sample, lists the two newest meetings under `Last meetings`, and links through to the full meetings list.

## Sub-features

- `home-load` renders the greeting and three insight cards after meetings fetch (`Meetings`, `In progress`, `Tasks`). Busy meetings raise the In progress count. There is no fourth card titled with a meeting `sourceId`.
...
- Sign-in has completed. The greeting includes `Verify`.
